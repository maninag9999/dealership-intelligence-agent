"""
Rewrites generators.py with the RepArchetype bug fixed.
Run from project root:
    uv run python fix_generators.py
"""
...
_ARCHETYPE_PARAMS: dict[str, dict] = {
    "high_volume":  {"close_rate_mean": 0.34, "close_rate_std": 0.04,
                     "margin_tendency_mean": -400.0,  "margin_tendency_std": 200.0,
                     "sat_bias": -0.15, "mix_weight": 0.25},
    "relationship": {"close_rate_mean": 0.22, "close_rate_std": 0.03,
                     "margin_tendency_mean":  900.0,  "margin_tendency_std": 300.0,
                     "sat_bias":  0.35, "mix_weight": 0.40},
    "struggling":   {"close_rate_mean": 0.11, "close_rate_std": 0.05,
                     "margin_tendency_mean": -1400.0, "margin_tendency_std": 500.0,
                     "sat_bias": -0.30, "mix_weight": 0.35},
}
    ...
    *,
    ...
    for _ in range(n):
        ...
        rows.append({
            ...
            "_archetype":         arch,
            "_base_close_rate":   round(float(np.clip(rng.normal(p["close_rate_mean"],
                                                                  p["close_rate_std"]), 0.05, 0.55)), 4),
            "_margin_tendency":   round(float(rng.normal(p["margin_tendency_mean"],
                                                          p["margin_tendency_std"])), 2),
        })
...
def generate_vehicles(
    ...
    *,
    ...
    for _ in range(n):
        ...
        arr   = _random_date(rng, reference_date - timedelta(days=90),
...
def generate_sales(
    ...
    *,
    ...
    va    = dict(zip(vehicles_df["vehicle_id"], vehicles_df["asking_price"]))
    vc    = dict(zip(vehicles_df["vehicle_id"], vehicles_df["cost_basis"]))
    vr    = dict(zip(vehicles_df["vehicle_id"], vehicles_df["arrived_date"]))
    vt    = dict(zip(vehicles_df["vehicle_id"], vehicles_df["_true_base_value"]))

    c_ids = customers_df["customer_id"].values
    ci    = dict(zip(customers_df["customer_id"], customers_df["income_bracket"]))
    cp    = dict(zip(customers_df["customer_id"], customers_df["_price_sensitivity"]))

    r_ids = reps_df["rep_id"].values
    ra    = dict(zip(reps_df["rep_id"], reps_df["_archetype"]))
    rm    = dict(zip(reps_df["rep_id"], reps_df["_margin_tendency"]))
    ...
    for _ in range(n):
        ...
        dp  = float(np.clip(rng.normal(0.04, 0.015) * ps * mm, 0.0, 0.18))
...
target = Path("src/dealership/ingestion/synthetic/generators.py")
target.write_text(content, encoding="utf-8")
print(f"Fixed: {target}")
print("\nNow run:")
print("  uv run python scripts/generate_synthetic_data.py")
print("  uv run python scripts/sanity_check_synthetic_data.py")
